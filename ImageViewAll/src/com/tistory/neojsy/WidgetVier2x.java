...
public class WidgetVier2x extends AppWidgetProvider {
    ...
    final static String folderListFileName = "wfoler2x.txt";
    final static String fileListFileName = "wfile2x.txt";
    ...
    final private static String WIDGETS = "widget2";
    ...
    public static void updateWidget(Context context, AppWidgetManager widgetMgr, int mAppWidgetId) {
        ...
        RemoteViews views = new RemoteViews(context.getPackageName(),R.layout.widgetviewer2);
        ...
        if(db.getValue(context, WIDGETS, "NO").equals("ZERO")){
            ...
            if(setFilePath()){
                views.setTextViewText(R.id.wbutton2, " ");
            WidgetBmpMaker bmpmaker = new WidgetBmpMaker(seletedFileFullPath, 360, 450, false);
            Bitmap bitmap = bmpmaker.getBitmap();
            if (bitmap != null)
                views.setImageViewBitmap(R.id.widgetimage2, bitmap);
        }
            else{
        ...
        PendingIntent testPendingIntent = PendingIntent.getBroadcast(context,0, new Intent(ACTION_WIDGET_IMAGECHANGE, Uri.EMPTY, context,WidgetVier2x.class), PendingIntent.FLAG_UPDATE_CURRENT);

        views.setOnClickPendingIntent(R.id.wbutton2, testPendingIntent);
