...
public class WidgetVier4x extends AppWidgetProvider {

    private static String ACTION_WIDGET_IMAGECHANGE = "change3";

    final static String folderListFileName = "wfoler4x.txt";
    final static String fileListFileName = "wfile4x.txt";
    ...
    final private static String WIDGETS = "widget3";
    ...
    public static void updateWidget(Context context, AppWidgetManager widgetMgr, int mAppWidgetId) {
        Log.d(null, "updateWidget");

        RemoteViews views = new RemoteViews(context.getPackageName(),R.layout.widgetviewer4);

        UseDb db = new UseDb();
        
        if(db.getValue(context, WIDGETS, "NO").equals("ZERO")){
            Log.d(null, "no image file");
            views.setTextViewText(R.id.wbutton2, "No Image File");
        }
            ...
            if(setFilePath()){
                views.setTextViewText(R.id.wbutton3, " ");
            WidgetBmpMaker bmpmaker = new WidgetBmpMaker(seletedFileFullPath, 480, 600, false);
            Bitmap bitmap = bmpmaker.getBitmap();
            if (bitmap != null)
                views.setImageViewBitmap(R.id.widgetimage3, bitmap);
        }
            else{
        ...
        PendingIntent testPendingIntent = PendingIntent.getBroadcast(context,0, new Intent(ACTION_WIDGET_IMAGECHANGE, Uri.EMPTY, context,WidgetVier4x.class), PendingIntent.FLAG_UPDATE_CURRENT);

        views.setOnClickPendingIntent(R.id.wbutton3, testPendingIntent);
        widgetMgr.updateAppWidget(mAppWidgetId, views);

    }

    public static void setList(Context context, String folder, String file,    int fileNumber) {
        Log.d(null, "setList");
        if (fileNumber != 0) {
            UseDb db = new UseDb();
